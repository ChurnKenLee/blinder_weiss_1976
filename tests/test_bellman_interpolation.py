...
def test_precomputed_slopes_and_direct_lookup_match_general_lookup() -> None:
    ...
    assets = np.concatenate(
        (asset_grid, np.nextafter(asset_grid, -np.inf), np.nextafter(asset_grid, np.inf))
    )
    logs = np.linspace(log_grid[0], log_grid[-1], assets.size)
    derivatives = pchip_slopes(jnp.asarray(asset_grid), values)
    expected = tensor_pchip_interpolate(
        values,
...
def test_gradients_are_continuous_across_knots_in_monotone_region() -> None:
    ...
    values = (
        jnp.log1p(assets[:, None])
        + 0.3 * jnp.exp(log_grid[None, :])
    ...
    expected_log_derivative = PchipInterpolator(np.asarray(log_grid), along_assets).derivative()(
        0.3
    )
...
def test_one_dimensional_shape_and_finite_autodiff(data: list[float]) -> None:
    ...
    derivative = jax.grad(
        lambda table: jnp.sum(tensor_pchip_interpolate(table, assets, log_grid, queries, 0.0))
    )(values)
    assert np.all(np.isfinite(np.asarray(derivative)))
    query_derivatives = jax.vmap(
        jax.grad(
            lambda query: tensor_pchip_interpolate(
