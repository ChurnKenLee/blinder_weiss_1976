...
def test_local_refinement_can_go_below_consumption_seed_minimum():
    ...
    assets, logs = jnp.meshgrid(
        jnp.asarray(refined.asset_grid), jnp.asarray(refined.log_human_capital_grid), indexing="ij"
    )
    capacity = np.asarray(
        maximum_feasible_consumption(
            assets,
            logs,
            jnp.asarray(refined.hours_policy[0]),
            jnp.asarray(refined.training_time_policy[0]),
            params,
            params.horizon,
            cfg.asset_minimum,
