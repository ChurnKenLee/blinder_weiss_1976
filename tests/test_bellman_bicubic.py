    ...
    for axis in (0, 1):
        ...
        for i in range(grid.size - 1):
            ...
            for j in range(other_grid.size - 1):
                ...
                ) / other_width - tolerance
                assert high_cross <= (
                    3.0 * high - width * max(sign * slopes[i, j + 1], sign * slopes[i + 1, j + 1])
                ) / other_width + tolerance
                for endpoint in (i, i + 1):
                    lower = -3.0 * sign * slopes[endpoint, j] / other_width
                    upper = 3.0 * (
                        low_cross / width + 3.0 * low / (width * other_width)
                        - sign * slopes[endpoint, j] / other_width
                    )
                    assert lower - tolerance <= sign * mixed[endpoint, j] <= upper + tolerance
                    lower = 3.0 * (
                        high_cross / width - 3.0 * high / (width * other_width)
...
@pytest.mark.parametrize("case", ["random", "retirement", "counterexample", "decreasing", "constant"])
def test_monotone_bicubic_constraints_and_dense_monotonicity(case: str) -> None:
    ...
    if case == "retirement":
        ...
        values = np.array([[0.0, 1.0, 2.0, 3.0],
                           [0.1, 1.001, 2.001, 3.001],
                           [0.2, 1.002, 2.002, 3.002]])
    elif case == "decreasing":
        values = values[::-1]
        direction[0] = -1.0
    elif case == "constant":
        values[:] = -3.0
    prepared = jax.jit(prepare_monotone_bicubic)(jnp.asarray(values), jnp.asarray(assets), jnp.asarray(logs))
    ...
    asset_mesh, log_mesh = np.meshgrid(np.linspace(assets[0], assets[-1], 101),
                                     np.linspace(logs[0], logs[-1], 97), indexing="ij")
    queries = jnp.asarray(np.stack((asset_mesh.ravel(), log_mesh.ravel()), axis=1))

    def evaluate(query):
        return monotone_bicubic_interpolate(prepared, jnp.asarray(assets), jnp.asarray(logs),
                                           query[0], query[1])
    ...
    reconstructed = monotone_bicubic_interpolate(prepared, jnp.asarray(assets), jnp.asarray(logs),
...
def test_bicubic_reproduces_bilinear_functions(asset_nodes: int, log_nodes: int) -> None:
    ...
    values = 2.0 + 1.1 * assets[:, None] + 0.7 * logs[None, :] + 0.2 * assets[:, None] * logs[None, :]
...
def test_bicubic_has_continuous_first_derivatives_across_both_cell_edges() -> None:
    ...
    for asset in np.asarray(assets)[1:-1]:
        np.testing.assert_allclose(gradient(jnp.asarray([asset - 1e-8, 0.2])),
                                   gradient(jnp.asarray([asset + 1e-8, 0.2])),
                                   rtol=1e-6, atol=2e-7)
    for log_value in np.asarray(logs)[1:-1]:
        np.testing.assert_allclose(gradient(jnp.asarray([0.8, log_value - 1e-8])),
...
def test_bicubic_direct_lookup_and_parameter_autodiff_are_finite() -> None:
    ...
    direct = monotone_bicubic_interpolate(packed, assets, logs, queries[:, 0], queries[:, 1],
                                        asset_grid_curvature=2.5, uniform_log_grid=True)
    ...
    def objective(scale):
        ...
        return jnp.sum(monotone_bicubic_interpolate(prepared, assets, logs,
                                                   queries[:, 0], queries[:, 1]))
