...
def test_direct_grid_indexing_matches_searchsorted_at_and_around_nodes(curvature):
    cfg = replace(BellmanConfig(), asset_nodes=41, human_capital_nodes=29,
                  asset_grid_curvature=curvature)
    a, y = bellman_state_grids(cfg)
    rng = np.random.default_rng(182)
    aq = np.concatenate([a, np.nextafter(a, -np.inf), np.nextafter(a, np.inf),
                         rng.uniform(a[0] - 1, a[-1] + 1, 1000)])
    yq = np.concatenate([y, np.nextafter(y, -np.inf), np.nextafter(y, np.inf),
    ...
    actual = jax.jit(lambda *xs: _interpolate_jax(
        *xs, asset_grid_curvature=curvature))(*args)
...
def test_direct_indexing_preserves_autodiff_inside_cells():
    ...
    def interpolate(z, curvature):
        return _interpolate_jax(values, jnp.asarray(a), jnp.asarray(y), z[0], z[1],
                                asset_grid_curvature=curvature)
    np.testing.assert_allclose(jax.grad(lambda z: interpolate(z, 2.0))(state),
