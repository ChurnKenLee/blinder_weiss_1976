...
def test_pchip_recursion_queries_and_diagnostics_use_same_value_representation():
    cfg = BellmanConfig(
        periods=3, asset_nodes=7, human_capital_nodes=5,
        hours_nodes=5, investment_nodes=5, consumption_nodes=5,
        refinement_steps=4, value_interpolation='pchip', compute_platform='cpu',
    )
    ...
    assert float(value_at(s, 0, assets, np.exp(log_k))) == pytest.approx(float(reference), rel=1e-12)
    assert s.initial_value == pytest.approx(float(value_at(
        s, 0, s.params.initial_assets, s.params.initial_human_capital)), rel=1e-12)


def test_terminal_analytic_bellman_step_is_independent_of_interpolation():
    cfg = BellmanConfig(
        periods=1, asset_nodes=5, human_capital_nodes=4,
        hours_nodes=3, investment_nodes=3, consumption_nodes=3,
        refinement_steps=3, compute_platform='cpu',
    )
    ...
    np.testing.assert_allclose(cubic.consumption_policy, linear.consumption_policy,
