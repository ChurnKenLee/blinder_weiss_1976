...
def main() -> None:
    ...
    cfg = BellmanConfig(
        compute_platform='gpu', asset_nodes=args.asset_nodes,
        human_capital_nodes=args.human_capital_nodes, periods=args.periods,
    ...
    report = dict(jax=jax.__version__, python=platform.python_version(), config=asdict(cfg),
                  timing_includes_host_transfer=True, results=[])
    solution = None
    ...
    arrays = dict(values=solution.values, c=solution.consumption_policy,
                  h=solution.hours_policy, q=solution.training_time_policy,
                  A=solution.asset_grid, y=solution.log_human_capital_grid,
                  time=solution.time)
