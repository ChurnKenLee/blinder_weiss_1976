...
def validate(directory):
    ...
    with gzip.open(archive, "rb") as stream:
        for record_count, raw in enumerate(stream, 1):
            ...
            def number(name, row=row, record_count=record_count):
                col = columns[name]
                try:
                    return int(row[col["start"] : col["end"]])
                except ValueError:
                    raise ValueError(
                        f"Invalid integer in {name} at record {record_count}"
                    ) from None

            weight_column = columns[weight_name]
            try:
                weight = Decimal(
                    row[weight_column["start"] : weight_column["end"]].decode().strip()
                )
    ...
    result = {
        ...
        "survey_weight_sum_unit": "persons"
        if collection == "usa"
        else "respondent-days; apply weights once per respondent",
