...
def calibration_stability_view(
    ...
    def _fit_number(value, precision=".3g"):
        return format(value, precision) if value is not None else "—"
    ...
    for _name, _run in calibration_stability_report.get("resolutions", {}).items():
        ...
        _calibration_rows.append({
            ...
            "Evaluations": len(_trace),
            "Initial loss": _fit_number(_run.get("baseline_loss_against_fixed_target")),
            "Selected parameter": _fit_number(_run.get("fitted_parameter"), ".6f"),
            "Selected loss": _fit_number(_run.get("fitted_loss")),
            "Selection": _run.get("selection_source", "pending"),
        })
    _acceptance_rows = []
    for _name, _check in calibration_stability_report.get("comparisons", {}).items():
        _acceptance_rows.append({
            ...
            "Moment change / scale": _fit_number(_check["maximum_standardized_moment_difference"]),
            "Loss change": _fit_number(_check["absolute_loss_difference"]),
            "Parameter change": _fit_number(_check["absolute_fitted_parameter_difference"]),
        })
